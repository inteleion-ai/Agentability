...
const POLL  = 60_000   // 1 min refresh

function q<T>(key: readonly unknown[], fn: () => Promise<T>, opts?: Partial<UseQueryOptions<T>>) {
  return useQuery<T>({ queryKey: key, queryFn: fn, staleTime: STALE, refetchInterval: POLL, ...opts })
}

// ── Hooks ────────────────────────────────────────────────────────
export const useHealth = () => q(QK.health, api.health)
