...
const DECISION_TYPES = ['classification','regression','generation','retrieval','planning','execution','delegation','coordination','routing','tool_selection']
...
function DetailPanel({ decision, onClose }: { decision: Decision; onClose: () => void }) {
  return (
    <div style={{
      ...
      zIndex: 100, overflowY: 'auto', padding: 20, display: 'flex', flexDirection: 'column', gap: 16,
    }}>
...
function Row({ label, value, mono, valueColor }: { label: string; value: string; mono?: boolean; valueColor?: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
      ...
      <span style={{ fontSize: 12, color: valueColor ?? 'var(--text-primary)', fontFamily: mono ? 'var(--font-mono)' : undefined, textAlign: 'right', wordBreak: 'break-all' }}>{value}</span>
    </div>
  )
}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8 }}>{title}</div>
      {children}
    </div>
  )
}
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, alignItems: 'center' }}>
        <input
          ...
          style={{
            ...
            color: 'var(--text-primary)', padding: '7px 12px', fontSize: 12, width: 220,
        ...
        {(agentFilter || typeFilter) && (
          <button
            onClick={() => { setAgentFilter(''); setTypeFilter(''); setPage(0) }}
            style={{ background: 'none', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text-muted)', padding: '7px 12px', fontSize: 12, cursor: 'pointer' }}
          >
            Clear
          </button>
        )}
                  ...
                  <tr key={d.decision_id} onClick={() => setSelected(d)} style={{ cursor: 'pointer' }}>
                    <td>
                      <span className="mono" style={{ fontSize: 12, color: 'var(--blue)' }}>
                        {truncate(d.agent_id, 24)}
                      </span>
                    </td>
                    <td>
                      <Badge variant="muted">{d.decision_type}</Badge>
                    </td>
                    <td>
      ...
      {(data?.meta.total ?? 0) > limit && (
        <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 10, marginTop: 12 }}>
          <button
            ...
            style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text-secondary)', padding: '6px 14px', fontSize: 12, cursor: page === 0 ? 'not-allowed' : 'pointer', opacity: page === 0 ? 0.4 : 1 }}
          >
            ← Prev
          </button>
          ...
          <button
            ...
            onClick={() => setPage((p) => p + 1)}
            style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text-secondary)', padding: '6px 14px', fontSize: 12, cursor: (page + 1) * limit >= (data?.meta.total ?? 0) ? 'not-allowed' : 'pointer', opacity: (page + 1) * limit >= (data?.meta.total ?? 0) ? 0.4 : 1 }}
          >
            Next →
          </button>
        </div>
