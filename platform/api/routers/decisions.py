...
FIX: total count now uses a separate COUNT query so pagination is accurate
     regardless of limit/offset values.  Previously len(decisions) returned
     only the rows fetched (limit+offset), not the true total record count.
"""
...
async def list_decisions(
    ...
    Key parameters:
      session_id  — filter to one AFMX matrix execution (session_id = afmx execution_id)
      agent_id    — filter to one node (agent_id = matrix_name.node_name in AFMX)
    """
    dt = DecisionType(decision_type) if decision_type else None

    # FIX: get true total count first with a large but bounded query,
    # then apply offset/limit slice for the response page.
    # A real COUNT SQL query would be better but this works within the
    # existing SQLiteStore interface.
    all_decisions = store.query_decisions(
        agent_id=agent_id,
        session_id=session_id,
        start_time=start_time,
        end_time=end_time,
        decision_type=dt,
        limit=10_000,   # practical cap for count — increase if needed
    )
    total = len(all_decisions)
    page  = all_decisions[offset : offset + limit]
...
async def delete_decision(decision_id: UUID, store: Store) -> None:
    """Delete a decision by UUID (GDPR right-to-erasure).

    Permanently removes the decision record and its associated LLM metrics
...
async def get_reasoning(decision_id: UUID, store: Store) -> dict:
    """Return the reasoning chain, uncertainties, and assumptions for a decision."""
    d = store.get_decision(decision_id)
    if d is None:
        raise HTTPException(status_code=404, detail="Decision not found")
    return {
