...
class SQLiteStore:
    ...
    def delete_decision(self, decision_id: UUID) -> None:
        """Permanently delete a decision and its linked LLM metrics.
        ...
        with self._lock:
            cursor = self.conn.cursor()
            cursor.execute(
                "DELETE FROM llm_metrics WHERE decision_id = ?",
                (str(decision_id),),
            )
            cursor.execute(
            ...
            self.conn.commit()
        logger.debug("Decision deleted: %s", decision_id)
