...
Usage (run from project root):
    python3.11 sdk/python/examples/seed_demo.py
    AGENTABILITY_DB=custom.db python3.11 sdk/python/examples/seed_demo.py
...
def rand_confidence(agent_id, ts):
    """Simulate a downward drift for risk_agent in the last 24 h."""
    base = {
        "risk_agent": 0.82, "sales_agent": 0.88,
        "compliance_agent": 0.91, "routing_agent": 0.79, "rag_agent": 0.85,
    }.get(agent_id, 0.80)
    age_hours = (datetime.utcnow() - ts).total_seconds() / 3600
    if agent_id == "risk_agent" and age_hours < 24:
        base -= 0.22  # simulate drift
    return max(0.10, min(0.99, base + random.gauss(0, 0.06)))


def seed(store):
    now = datetime.utcnow()
    decisions_written = llm_written = mem_written = conflict_written = 0

    for agent_id in AGENTS:
        for hour_back in range(0, 168, 2):
            for _ in range(random.randint(3, 12)):
                ...
                prov = random.randint(100, 4000)
                comp = random.randint(50, 800)
                provider, model, inp_price, out_price = random.choice(MODELS)
                tok = prov + comp
                cost = (prov / 1e6) * inp_price + (comp / 1e6) * out_price
                violated = (["income_freshness_days <= 30"]
                             if conf < 0.5 and random.random() < 0.4 else [])

                d = Decision(
                    agent_id=agent_id,
                    session_id="session_{}_{:d}".format(agent_id, hour_back // 24),
                    timestamp=ts,
                    latency_ms=latency,
                    decision_type=random.choice(DECISION_TYPES),
                    input_data={"query": "sample input {:d}".format(random.randint(1, 999))},
                    output_data={"result": random.choice(["approve", "deny", "escalate", "route"])},
                    confidence=conf,
                    reasoning=[
                        "Step {:d}: evaluated {}".format(
                            i + 1, random.choice(["credit", "income", "policy", "context"])
                        )
                    ...
                    data_sources=random.sample(
                        ["credit_bureau", "income_api", "vector_store", "policy_db"],
    ...
    for _ in range(80):
        ...
        ts = now - timedelta(hours=random.uniform(0, 168))
        c = AgentConflict(
            session_id="session_conflict_{:d}".format(random.randint(1, 20)),
            timestamp=ts,
            ...
            resolution_strategy=random.choice(
                ["priority_hierarchy", "confidence_based", "consensus"]
            ),
        )
    ...
    print("Seed complete:")
    print("  {:6d} decisions".format(decisions_written))
    print("  {:6d} LLM calls".format(llm_written))
    print("  {:6d} memory operations".format(mem_written))
    print("  {:6d} conflicts".format(conflict_written))
